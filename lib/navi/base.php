...
class NaviBase {
    ...
    public function run() {
        ...
        if (isset($_GET['router'])) {
            ...
            if (isset($routers[0])) {
                $filepath =/* NaviBase::$baseUrl . */'application' . NaviBase::$config['ds'] . strtolower($module)  . NaviBase::$config['ds'] . 'controller' . NaviBase::$config['ds'] . str_replace('-', '', $routers[0]) . '.php';
    ...
    /**
     * load mang array trong application vao 
     * 
     * @param type $_config
     * @return \self
     */
